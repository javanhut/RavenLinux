...
use raven_core::{DockDiff, DockItem, ShellCommand};
...
impl Dock {
    pub fn new(command_tx: mpsc::Sender<ShellCommand>, vertical: bool) -> Self {
        ...
        btn.connect_clicked(move |_| {
            if running {
                if let Some(ref addr) = address {
                    let _ = tx.blocking_send(ShellCommand::FocusWindow(addr.clone()));
                }
            } else {
                let _ = tx.blocking_send(ShellCommand::LaunchApp(command.clone()));
            }
        });
    ...
    fn show_context_menu(
        ...
        pin_btn.connect_clicked(move |_| {
            let _ = tx_pin.blocking_send(ShellCommand::PinApp {
                id: id_pin.clone(),
                pinned: !pinned,
            });
            let _ = tx_pin.blocking_send(ShellCommand::SaveDockConfig);
            ...
            if let Some(addr) = address {
                ...
                min_btn.connect_clicked(move |_| {
                    let cmd = if minimized {
                        ShellCommand::RestoreWindow(addr_min.clone())
                    } else {
                        ShellCommand::MinimizeWindow(addr_min.clone())
                    };
                ...
                close_btn.connect_clicked(move |_| {
                    let _ = tx_close.blocking_send(ShellCommand::CloseWindow(addr_close.clone()));
                    popover_clone.popdown();
                });
