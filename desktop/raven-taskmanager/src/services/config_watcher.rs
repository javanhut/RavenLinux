...
impl ConfigWatcher {
    pub fn new(paths: ConfigPaths, event_tx: Sender<PanelEvent>) -> Self {
        Self { paths, event_tx }
    }
        ...
        match tokio::fs::read(&self.paths.dock_config).await {
            Ok(data) => match serde_json::from_slice::<PanelConfig>(&data) {
                Ok(config) => {
                    ...
                    let _ = self.event_tx.send(PanelEvent::ConfigReloaded(config)).await;
                }
                Err(e) => {
        ...
        match tokio::fs::read(&self.paths.raven_settings).await {
            Ok(data) => match serde_json::from_slice::<RavenSettings>(&data) {
                Ok(settings) => {
                    ...
                    let _ = self.event_tx.send(PanelEvent::SettingsReloaded(settings)).await;
                }
                Err(e) => {
