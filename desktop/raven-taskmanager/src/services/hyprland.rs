...
impl HyprlandService {
    pub fn new(event_tx: Sender<PanelEvent>, command_rx: mpsc::Receiver<PanelCommand>) -> Self {
        Self {
            event_tx,
            command_rx,
        }
        ...
        let listener_handle = tokio::spawn(async move {
            loop {
                if let Err(e) = Self::run_event_listener(event_tx.clone()).await {
                    ...
                    let _ = event_tx.send(PanelEvent::HyprlandDisconnected).await;
                    tokio::time::sleep(std::time::Duration::from_secs(2)).await;
                }
            }
        });
    ...
    async fn try_connect(&self) -> anyhow::Result<()> {
        ...
        let _ = self.event_tx.send(PanelEvent::HyprlandConnected).await;
    ...
    async fn run_event_listener(tx: Sender<PanelEvent>) -> anyhow::Result<()> {
        let mut listener = EventListener::new();

        // Window opened
        let tx1 = tx.clone();
        listener.add_window_open_handler(move |data| {
            let event = PanelEvent::WindowOpened {
        ...
        listener.add_window_close_handler(move |addr| {
            let event = PanelEvent::WindowClosed {
        ...
        listener.add_active_window_change_handler(move |data| {
            if let Some(data) = data {
                let event = PanelEvent::WindowFocused {
                    address: data.window_address.to_string().into(),
                };
        ...
        listener.add_window_moved_handler(move |data| {
            ...
            let event = PanelEvent::WindowMoved {
                address: data.window_address.to_string().into(),
        ...
        // Window title changed - in 0.3 API this only gives the address
        let tx5 = tx.clone();
        listener.add_window_title_change_handler(move |addr| {
            let event = PanelEvent::WindowTitleChanged {
                address: addr.to_string().into(),
    ...
    async fn handle_command(&self, cmd: PanelCommand) -> anyhow::Result<()> {
        match cmd {
            PanelCommand::FocusWindow(addr) => {
            ...
            PanelCommand::MinimizeWindow(addr) => {
            ...
            PanelCommand::RestoreWindow(addr) => {
            ...
            // These are handled by ProcessService
...
/// Helper to convert window address string to Hyprland Address type
fn parse_address(addr: &CompactString) -> Address {
    Address::new(addr.as_str())
}
