...
impl DockState {
    ...
    pub fn apply_event(&mut self, event: &PanelEvent) -> SmallVec<[DockDiff; 4]> {
        let mut diffs = SmallVec::new();

        match event {
            PanelEvent::WindowOpened {
                address,
                class,
                title,
                pid,
            ...
            PanelEvent::WindowClosed { address } => {
                let id: CompactString = format!("hypr-{}", address).into();
            ...
            PanelEvent::WindowFocused { address } => {
            ...
            PanelEvent::WindowTitleChanged { address, title } => {
                let id: CompactString = format!("hypr-{}", address).into();
            ...
            PanelEvent::ConfigReloaded(config) => {
                // Merge pinned apps from new config
                // First, mark all current pinned as unpinned
                for item in self.items.values_mut() {
                    if item.pinned && !item.running {
                        // Will be removed if not in new config
                    }
                }
