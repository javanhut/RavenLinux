...
impl PowerMenu {
    pub fn new(
        ...
        logout_btn.connect_clicked(move |_| {
            let _ = tx.blocking_send(PanelCommand::Logout);
            win.close();
        });
        ...
        lock_btn.connect_clicked(move |_| {
            let _ = tx.blocking_send(PanelCommand::Lock);
            win.close();
        });
        ...
        reboot_btn.connect_clicked(move |_| {
            let _ = tx.blocking_send(PanelCommand::Reboot);
            win.close();
        });
        ...
        shutdown_btn.connect_clicked(move |_| {
            let _ = tx.blocking_send(PanelCommand::Shutdown);
            win.close();
        });
