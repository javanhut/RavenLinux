...
Raven Shell is built with a modern async architecture using:
- **Rust** with GTK4 for the UI
- **Tokio** for async runtime
- **hyprland-rs** for event-driven Hyprland IPC
- **gtk4-layer-shell** for Wayland layer-shell support

### Key Improvements Over Previous Implementation

| Feature | Previous (Go) | Current (Rust) |
|---------|---------------|----------------|
| Window Updates | 1500ms polling | Event-driven (<10ms) |
| CPU Usage | Constant polling | Zero when idle |
| Process Spawning | fork+exec per command | Direct socket IPC |
| Dock Updates | Full widget recreation | Differential updates |
| Memory | New allocations per cycle | Reused buffers |
...
The dock uses Hyprland's event socket for instant window detection. Running applications are indicated by a teal underline. Focused windows have an additional highlight.
...
The dock uses Hyprland's native IPC socket for event-driven window management:

### Event Socket
Connected to `$XDG_RUNTIME_DIR/hypr/$HYPRLAND_INSTANCE_SIGNATURE/.socket2.sock` for real-time events:
- `openwindow` - Window opened
- `closewindow` - Window closed
- `activewindow` - Focus changed
- `movewindow` - Window moved (including minimize/restore)
- `windowtitle` - Title changed

### Command Socket
Used for window operations via hyprland-rs dispatch API:

| Action | Hyprland Dispatch |
|--------|-------------------|
| Focus window | `FocusWindow(Address)` |
| Close window | `CloseWindow(Address)` |
| Minimize | `MoveToWorkspaceSilent(Special("minimized"))` |
| Restore | `MoveToWorkspaceSilent(Relative(0))` |
| Logout | `Exit` |

### Requirements
- Hyprland compositor must be running
- hyprland-rs compatible (Hyprland 0.40+)
...
- **Focused app**: Teal background highlight + brighter border
...
- `.dock-item-focused`: Currently focused window
...
- `raven-shell` - The panel application
- `raven-ctl` - CLI window control utility

## Environment Variables

| Variable | Description |
|----------|-------------|
| `RUST_LOG` | Log level (e.g., `raven_shell=debug`) |
| `XDG_CONFIG_HOME` | Config directory base |
| `XDG_RUNTIME_DIR` | Runtime directory for Hyprland socket |
