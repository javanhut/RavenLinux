...
#
# The ffmpeg/clang/lld group is Owl Player's and nothing else's, and it is the
# only per-application entry in these lists. The player is a front end for
# libavformat and libavcodec rather than a GStreamer widget, so its build needs
# FFmpeg's headers and .pc files; ffmpeg-sys-next then reads those headers with
# bindgen, which loads libclang.so; and the checkout's own .cargo/config.toml
# links with lld. Only the first two are required -- stage_player() withdraws
# the lld request on a host without it -- and all three are checked before the
