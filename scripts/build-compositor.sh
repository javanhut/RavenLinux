...
# Build raven-shell (Rust workspace - unified panel, desktop, menu, settings)
SHELL_DIR="${PROJECT_ROOT}/desktop/raven-shell"
if [[ -d "$SHELL_DIR" ]]; then
    echo ""
    echo "=========================================="
    echo "  Building Raven Shell (Rust)"
    echo "=========================================="
    echo ""
    ...
    if cargo build --release 2>&1; then
        cp target/release/raven-shell "$OUTPUT_DIR/" 2>/dev/null || true
        cp target/release/raven-ctl "$OUTPUT_DIR/" 2>/dev/null || true
