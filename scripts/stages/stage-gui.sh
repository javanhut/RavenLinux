...
# against the set.
# RavenGUI's protocol bindings are generated by a proc macro, and cargo cannot
# see the file they are generated from.
#
# crates/raven-protocol/src/lib.rs calls
# `wayland_scanner::generate_server_code!("../../protocols/raven-shell-v1.xml")`.
# There is no build.rs in that workspace and no include_bytes! of the XML, so
# cargo's staleness check for `raven-protocol` looks at its .rs files and
# nothing else. Change the protocol, and cargo reuses the compiled rlib.
#
# What that produces is not a build failure in the crate that is wrong. It is a
# build failure in `huginn-comp`, which *did* change, compiling against
# bindings that did not:
#
#     error[E0432]: unresolved import
#       `raven_protocol::server::raven_output_layout_v1`
#     error[E0599]: no variant named `GetOutputLayout` found for enum
#       `raven_protocol::server::raven_shell_manager_v1::Request`
#
# -- with `Compiling huginn-comp` in the log and no `Compiling raven-protocol`
# above it, which is the tell.
#
# The XML is newer than lib.rs exactly when git has just written a new one, so
# touching lib.rs then is precise: it forces the rebuild on the run after the
# protocol moved and on no other run. The real fix belongs upstream -- one
# `include_bytes!` of the XML in that lib.rs would make cargo track it -- and
# this stops costing anything the day that lands.
refresh_generated_protocol() {
    local src="$1"
    local xml="${src}/protocols/raven-shell-v1.xml"
    local lib="${src}/crates/raven-protocol/src/lib.rs"

    [[ -f "${xml}" && -f "${lib}" ]] || return 0

    if [[ "${xml}" -nt "${lib}" ]]; then
        log_info "  raven-shell-v1.xml is newer than the crate that reads it;"
        log_info "  forcing raven-protocol to regenerate its bindings"
        touch "${lib}" 2>/dev/null || true
    fi
}
...
build_gui_workspace() {
    ...
    refresh_generated_protocol "${src}"
