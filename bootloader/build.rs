...
//! # The mark
//!
//! The same file also carries the raven mark, for the same reason: it is
//! `branding/raven-logo-light.png` at the repository root, and RavenBoot can
//! neither open it nor inflate it. It is decoded here, shrunk to
//! [`MARK_PX`] square, and stored as straight RGBA for `src/mark.rs` to
//! include. The light variant, because the mark sits on `theme::BACKDROP`.
//!
//! # Output
//!
//! * `$OUT_DIR/atlas.bin` — every glyph's 8-bit coverage, tightly packed.
//! * `$OUT_DIR/atlas.rs` — the index, included by `src/font.rs`.
//! * `$OUT_DIR/mark.bin` — the raven, `MARK_PX` x `MARK_PX` RGBA.
//! * `$OUT_DIR/mark.rs` — its size, included by `src/mark.rs`.
...
/// The raven mark's stored size, in pixels square.
///
/// The largest it is ever drawn is `theme::MARK_SIZE` (56) at `scale_for`'s
/// top scale of 2.0, so 128 means the menu only ever shrinks it, and
/// `gfx::Canvas::image` shrinks well. At four bytes a pixel it is 64 KiB of
/// `.rodata`, which is about what one of the eleven font sizes costs.
pub const MARK_PX: usize = 128;
...
/// Decode the branding master and shrink it to [`MARK_PX`].
fn mark(manifest: &std::path::Path, out: &std::path::Path) {
    let path = find_up(manifest, "branding/raven-logo-light.png").unwrap_or_else(|| {
        panic!(
            "no branding/raven-logo-light.png above {}",
            manifest.display()
        )
    });
    ...
    let mut reader = decoder.read_info().expect("not a usable PNG");
    let mut buf = vec![0u8; reader.output_buffer_size()];
    let info = reader.next_frame(&mut buf).expect("cannot decode PNG");
    assert_eq!(
        info.color_type,
        png::ColorType::Rgba,
        "the mark must be RGBA"
    );
    let (sw, sh) = (info.width as usize, info.height as usize);
    let src = &buf[..info.buffer_size()];

    // Box filter, premultiplied, so a transparent pixel's colour never leaks
    // into the average of its opaque neighbours.
    let n = MARK_PX;
    let mut dst = vec![0u8; n * n * 4];
    for y in 0..n {
        let (y0, y1) = (y * sh / n, ((y + 1) * sh / n).max(y * sh / n + 1));
        for x in 0..n {
            let (x0, x1) = (x * sw / n, ((x + 1) * sw / n).max(x * sw / n + 1));
            let (mut r, mut g, mut b, mut a, mut count) = (0.0f64, 0.0f64, 0.0f64, 0.0f64, 0.0f64);
            for sy in y0..y1.min(sh) {
                for sx in x0..x1.min(sw) {
                    let p = &src[(sy * sw + sx) * 4..][..4];
                    let alpha = f64::from(p[3]) / 255.0;
                    r += f64::from(p[0]) * alpha;
                    g += f64::from(p[1]) * alpha;
                    b += f64::from(p[2]) * alpha;
                    a += alpha;
                    count += 1.0;
                }
            }
            let o = &mut dst[(y * n + x) * 4..][..4];
            if a > 0.0 {
                o[0] = (r / a + 0.5) as u8;
                o[1] = (g / a + 0.5) as u8;
                o[2] = (b / a + 0.5) as u8;
                o[3] = (a / count * 255.0 + 0.5) as u8;
            }
        }
    }

    std::fs::write(out.join("mark.bin"), &dst).expect("cannot write mark.bin");
    std::fs::write(
        out.join("mark.rs"),
        format!(
            "/// Generated by build.rs from {}.\npub const MARK_PX: usize = {n};\n",
            path.display()
        ),
    )
    .expect("cannot write mark.rs");
}

/// The repository's font, from anywhere below the repository root.
pub fn find_font(start: &std::path::Path) -> Option<PathBuf> {
    find_up(start, "fonts/JetBrainsMonoNerdFontMono-Regular.ttf")
}

/// `relative`, from the nearest ancestor of `start` that has it.
pub fn find_up(start: &std::path::Path, relative: &str) -> Option<PathBuf> {
    let mut dir = Some(start);
    while let Some(here) = dir {
        let candidate = here.join(relative);
